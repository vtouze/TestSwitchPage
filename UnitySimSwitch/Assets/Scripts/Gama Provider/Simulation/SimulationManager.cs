...
public class SimulationManager : MonoBehaviour
{
    ...
    void GenerateGeometries(bool initGame, List<string> toRemove)
    {
        ...
        if (loadPlayerPositionFromGAMA && infoWorld.position != null && infoWorld.position.Count > 1 && (initGame || !sendMessageToReactivatePositionSent))
        {
    ...
    private void UpdatePlayerPosition()
    {
      ...
      //  Vector3 v = new Vector3(Camera.main.transform.position.x, Camera.main.transform.position.y - yOffsetCamera, Camera.main.transform.position.z);
        Vector3 v = new Vector3(player.transform.localPosition.x, player.transform.localPosition.y, player.transform.localPosition.z);
